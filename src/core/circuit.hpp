...
public:
  ...
  void invert_gate_type(int idx); // Invert gate type: AND↔NAND, OR↔NOR, NOT↔BUFF, XOR↔XNOR.
  ...
  bool rename_node(int idx, const std::string& new_name, std::string* error); // Rename a node, updating PO names when needed.
